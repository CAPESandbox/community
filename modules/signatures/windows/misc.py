...
class SuspiciousExecutionViaMicrosoftExchangeTransportAgent(Signature):
    ...
    description = "Attempts to execute a suspicious commands via MicrosoftExchangeTransportAgent for persistence or or malicious activities"
    severity = 3
    ...
    evented = True
    ttps = ["T1505.002"]
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/persistence_suspicious_execution_via_microsoft_exchange_transport_agent.toml"]
    ...
    def on_call(self, call, process):
        ...
        if pname in ["msexchangetransport.exe", "edgetransport.exe"]:
            if call["api"] == "CreateProcessInternalW":
                ...
                lower = cmdline.lower()
                if any(process in lower for process in ["wscript.exe", "cscript.exe", "mshta.exe", "certutil.exe",
                                                "certreq.exe", "bitsadmin.exe", "curl.exe", "reg.exe", "net.exe"]):
                    self.detected = True
                    return
...
class SuspiciousScheduledTaskCreationviaMasqueradedXMLFile(Signature):
    ...
    def on_call(self, call, process):
        ...
        if pname in ["setup_msi.exe", "setupactions.exe", "admsetupactions.exe","antimalware.exe",
                     "pcdrcui.exe", "setupactions.exe", "setupactions.exe", "wincompose.exe"]:
            if call["api"] == "CreateProcessInternalW":
                cmdline = self.get_argument(call, "CommandLine")
                lower = cmdline.lower()
    ...
    def on_complete(self):
        ...
        for cmdline in cmdlines:
            lower = cmdline.lower()
            if ("schtasks.exe" in lower and any(arg in lower for arg in ["/create", "-create"]) and
                    any(arg in lower for arg in ["/xml", "-xml"])):
                return True
        return False
...
class PotentialProtocolTunnelingViaLegitUtilities(Signature):
    ...
    minimum = "1.2"
    ttps = ["T1572"]
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/command_and_control_potential_protocol_tunneling_via_legit_utilities.toml"]
    evented = True
    ...
    def run(self):
        ...
        for cmdline in cmdlines:
            lower = cmdline.lower()
            if ("3389" in lower and any(arg in lower for arg in ["-L", "-P", "-R", "-pw", "-ssh"])
                    or "ssh.exe" in lower and any(arg in lower for arg in ["127.0.0", "localhost"])
                    or "ngrok" in lower and any(arg in lower for arg in ["http", "tcp", "tunnel", "tls", "start", "3389"])):
                return True
...
class PotentialProtocolTunnelingViaQEMU(Signature):
    ...
    minimum = "1.2"
    ttps = ["T1572"]
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/command_and_control_potential_traffic_tunneling_with_qemu.toml"]
    evented = True
...
class PotentialLocationDiscoveryViaUnusualProcess(Signature):
    ...
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/discovery_potential_hawkeyes_stealer_infection.toml"]
    evented = True
    ...
    def on_call(self, call, process):
        ...
        if (not "\\endpoint protection sdk\\endpointprotection.exe" in pnameFullPath) or (not "\\aemagent\\rmm.advancedthreatdetection\\dattoav\\endpoint protection sdk\\endpointprotection.exe" in pnameFullPath):
            if call["api"] == "CreateProcessInternalW":
                if call["api"] == "CreateProcessInternalW":
                    cmdline = self.get_argument(call, "CommandLine")
                    lower = cmdline.lower()
                    if (any(process in lower for process in ["chrome.exe", "msedge.exe", "brave.exe",
                                                            "browser.exe", "dragon.exe"]) and "--dump-dom" in lower and
                            "http" in lower):
                        self.detected = True
...
class ExecutionFromSelfExtractingArchive(Signature):
    ...
    minimum = "1.2"
    ttps = ["T1204"]
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/execution_execution_from_a_password_protected_self_extracting_archive.toml"]
    evented = True
...
class SuspiciousJavaExecutionViaWinScripts(Signature):
    ...
    minimum = "1.2"
    ttps = ["T1059"]
    references = ["https://github.com/elastic/protections-artifacts/blob/main/behavior/rules/windows/execution_suspicious_java_execution_via_a_windows_script.toml"]
    evented = True
