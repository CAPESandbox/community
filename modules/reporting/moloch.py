    ...
    def run(self, results):
        ...
        if "signatures" in results and results["signatures"]:
            for entry in results["signatures"]:
                cmd = cmd + ' -t "cuckoosig:%s:%s"' % (
                    re.sub(r"[\W]", "_", str(entry["name"])),
                    re.sub(r"[\W]", "_", str(entry["severity"])),
                )
        try:
        ...
        if "suricata" in results and results["suricata"]:
                ...
                for alert in results["suricata"]["alerts"]:
                    ...
                    if proto:
                        ...
                        if proto == "UDP" or proto == "TCP" or proto == "6" or proto == "17":
                            ...
                            tmpdict[
                                "expression"
                            ] = 'ip==%s && ip==%s && port==%s && port==%s && tags=="%s:%s" && ip.protocol==%s' % (
                                tmpdict["srcip"],
                                tmpdict["dstip"],
                                tmpdict["srcport"],
                                tmpdict["dstport"],
                            ...
                            tmpdict["cproto"] = "icmp"
                            tmpdict["nproto"] = 1
                            tmpdict["expression"] = 'ip==%s && ip==%s && tags=="%s:%s" && ip.protocol==%s' % (
                                tmpdict["srcip"],
                                tmpdict["dstip"],
                                self.CUCKOO_INSTANCE_TAG,
                                self.task_id,
                        ...
                        if tmpdict["hash"] in self.alerthash:
                            if alert["sid"] not in self.alerthash[tmpdict["hash"]]["sids"]:
                                ...
                                self.alerthash[tmpdict["hash"]]["msgs"].append(
                                    "suri_msg:%s" % (re.sub(r"[\W]", "_", alert["signature"]))
                                )
                        else:
            ...
            if "files" in results["suricata"]:
                        ...
                        if proto:
                            ...
                            if (
                                ...
                                self.fileshash[tmpdict["hash"]]["clamav"].append(
                                    "clamav:%s" % (re.sub(r"[\W]", "_", entry["file_info"]["clamav"]))
                                )
                            if (
                                entry["file_info"]["md5"]
                ...
                for entry in self.fileshash:
                    tags = ",".join(
                        list(map(str, self.fileshash[entry]["clamav"]))
                        + list(map(str, self.fileshash[entry]["md5"]))
                        + list(map(str, self.fileshash[entry]["sha256"]))
                        + list(map(str, self.fileshash[entry]["yara"]))
                    )
                    if tags:
